import type { MotelConfig, Room, RoomPricing } from "@workspace/motel-config";

// System-prompt builder — Layer 2 of the multi-motel architecture.
//
// The TEXT of this template (conversation flow, availability/safety rules,
// anti-tool-leak guards, multi-room logic) is behavior shared by every motel
// and stays in code; all motel-specific DATA is injected from MotelConfig.
// The golden snapshot test (system-prompt.test.ts) pins the exact output for
// Le Refuge — treat any diff there as a behavior change to justify.
//
// A few example dialogues still contain Refuge-flavored prose (casual room
// mentions like "the Double room", the glass-shower aside, the Jerry's
// Pizzéria sample answer): kept verbatim in this phase to guarantee a
// byte-identical prompt; generalizing the examples is phase-2 work.

function weekdayPrice(room: Room): number {
  return "flat" in room.pricing ? room.pricing.flat : room.pricing.weekday;
}

function weekendPrice(room: Room): number {
  return "flat" in room.pricing ? room.pricing.flat : room.pricing.weekend;
}

function renderPricing(pricing: RoomPricing): string {
  return "flat" in pricing
    ? `$${pricing.flat}/night all days`
    : `$${pricing.weekday}/weekday | $${pricing.weekend}/Fri-Sat`;
}

function renderRoom(room: Room): string {
  const guests = `max ${room.maxGuests} guests${room.bedding ? `, ${room.bedding}` : ""}`;
  const special = room.special ? `\nSpecial: ${room.special}` : "";
  return `${room.name} — ${renderPricing(room.pricing)} (${guests})${special}\nAmenities: ${room.amenities.join(", ")}`;
}

function roomById(config: MotelConfig, id: string): Room {
  const room = config.rooms.find((r) => r.id === id);
  if (!room) {
    throw new Error(`motel config "${config.id}": roomRoles references unknown room id "${id}"`);
  }
  return room;
}

function formatCurrentDate(config: MotelConfig, now: Date): string {
...
// Layer 3 boundary: tone notes are the ONLY free-text a motel config may add
// to the prompt, and they stay small so they can never crowd out or override
// the shared behavior rules above them.
const TONE_NOTES_MAX_LENGTH = 500;
...
export function buildSystemPromptBody(config: MotelConfig): string {
  const { identity, hours, policies, booking, attractions, personalization } = config;
  const phone = identity.phone;
  const coupleDefault = roomById(config, config.roomRoles.coupleDefault);
  const coupleUpgrade = roomById(config, config.roomRoles.coupleUpgrade);
  const cheapest = roomById(config, config.roomRoles.cheapest);
  const [singleOptionA, singleOptionB] = config.roomRoles.groupSingleRoomOptions.map((id) =>
    roomById(config, id)
  );
  const roomsBlock = config.rooms.map(renderRoom).join("\n\n");
  const thingsToDo = attractions.thingsToDo.map((a) => `- ${a.name} — ${a.note}`).join("\n");
  const whereToEat = attractions.dining.map((a) => `- ${a.name} — ${a.note}`).join("\n");

  if (personalization.toneNotes.length > TONE_NOTES_MAX_LENGTH) {
    throw new Error(
      `motel config "${config.id}": personalization.toneNotes exceeds ${TONE_NOTES_MAX_LENGTH} characters`
    );
  }
  const toneNotesBlock = personalization.toneNotes ? `\n\n${personalization.toneNotes}` : "";

  return `You are the intelligent AI receptionist for ${identity.name} in ${identity.city}, ${identity.region}.
Business hours when a live person is available: ${hours.receptionLabel}. Phone: ${phone}.
...
Good example:
Guest: '3' (nights)
Agent: [calls check_availability internally, gets the result] 'Parfait, du 10 au 13 septembre pour 2 personnes — c'est disponible ! Pour un couple, je recommande la Chambre Queen à ${weekdayPrice(coupleDefault)}$/nuit, notre option la plus abordable et confortable. Ça vous convient, ou vous préférez voir d'autres options ?'
...
CHECK-IN / CHECK-OUT:
- Standard check-in: ${hours.checkIn}
- Standard check-out: ${hours.checkOut}
- Late arrivals (after ${hours.lateArrivalCutoff}): MUST call ${phone} before ${hours.lateArrivalCutoff} that day — self check-in key location provided

PETS (${policies.pets.allowed}):
- $${policies.pets.deposit} refundable damage deposit required
- Rules: ${policies.pets.rules.join(" | ")}
- ${policies.pets.forfeitNote}

PARKING:
- ${policies.parking.standard}
- ${policies.parking.special}: ${phone}

ACCESSIBILITY / SPECIAL REQUESTS:
- ${policies.accessibility}: ${phone} (${hours.shortLabel})

EXTENDED STAYS (${policies.extendedStays.thresholdNights}+ nights):
- Quote the standard nightly rate upfront, exactly as you would for any shorter stay. There are no special discounts.
- Only if the guest specifically negotiates or asks for a lower/special rate: you may mention they can call ${phone} (${hours.shortLabel}) to speak with the team directly. Do NOT imply that exceptions or special deals are possible — our rate is the same regardless of stay length.

NON-SMOKING: ${policies.nonSmoking}
CANCELLATIONS / CHANGES: ${policies.cancellations} — call ${phone}
...
For a couple (2 people) with no stated preference, default to recommending the ${coupleDefault.name} first (our most affordable option). Only mention the ${coupleUpgrade.name} as a nice upgrade if they seem open to spending a bit more, or if they ask for something special/romantic — present it with a brief comparison (e.g. price difference and the glass shower). Don't lead with the ${coupleUpgrade.name} by default.
For groups (5+ people): identify how they want to split naturally in conversation, then offer the rooms that fit.
Good example: "With 6 people, we could set you up nicely — a Double room fits ${singleOptionA.maxGuests} people at $${weekendPrice(singleOptionA)}/night, and a Queen room would take care of the other 2 at $${weekendPrice(coupleDefault)}/night. Does that work for your group?"
...
Agent: "If you'd like to keep everyone in one room, you've got two options: the Double room at $${weekdayPrice(singleOptionA)}-${weekendPrice(singleOptionA)}/night (two beds, simple and budget-friendly), or the Suite at $${weekdayPrice(singleOptionB)}/night (queen bed + sofa bed, plus a full kitchenette if you'd like extra space and comfort). Which sounds better for your group?"
...
"Thank you for booking with ${identity.name}!

Here is what you need to know:
Address: ${identity.address}
Check-in: ${hours.checkIn}
Check-out: ${hours.checkOut}

For questions or to modify your reservation, call us at ${phone}.
Business hours: ${hours.postBookingLabel}.
...
"I used 2 nights — you can adjust on the booking page, or call ${phone} to modify."
...
FORMAT (English):
${booking.linkBase}?lang=EN&hotelid=${booking.hotelId}&fday=DD&fmonth=MM&fyear=YYYY&nbnights=NN&nbadt=ZZ

FORMAT (French):
${booking.linkBase}?lang=FR&hotelid=${booking.hotelId}&fday=DD&fmonth=MM&fyear=YYYY&nbnights=NN&nbadt=ZZ
...
${booking.linkBase}?lang=EN&hotelid=${booking.hotelId}&fday=25&fmonth=06&fyear=2026&nbnights=2&nbadt=2

Do NOT claim to confirm real-time availability — the link shows available options on the booking page.
If the guest skips the link: give ${identity.website} or ${phone}.

## Special Scenarios

Guest: "What's the cheapest?"
→ "Our most affordable option is the ${cheapest.name} at $${weekdayPrice(cheapest)}/night on weekdays. How many people will be staying?"

Guest needs accessibility:
→ "For accessibility needs, please call us at ${phone} — our team will find the best setup for you. Office hours: ${hours.shortLabel} daily."
...
(Only mention ${phone} if the guest pushes further or asks to negotiate.)

Late arrival (after ${hours.lateArrivalCutoff}):
→ "Late arrivals after ${hours.lateArrivalCutoff} require a phone call before ${hours.lateArrivalCutoff} that day. Call ${phone} and we will arrange self check-in and leave your key."
...
✅ For groups: help them decide how to split. For DIFFERENT room types you may offer one link per type; for several rooms of the SAME type, direct them to ${phone} (see Multiple Rooms)
✅ For pets: state $${policies.pets.deposit} deposit + all rules clearly
...
- Overwhelm with choices — suggest naturally and confirm${toneNotesBlock}
...
- partial: tell the guest concisely which dates are booked versus open, offer a link for the available stretch, and mention the team can help arrange the rest at ${phone}. For example: "Quick heads-up — July 3rd and 4th are fully booked, but the rest of your dates are open. I can set you up with a link for July 1st–2nd to start, and our team can help arrange the rest at ${phone}. Want me to do that?"
- none_available: "Unfortunately those exact dates are fully booked. Would different dates work? Or you can call us at ${phone} and we'll find the best option."
- too_long: "For a stay that long, the best way is to call us directly at ${phone} — our team will sort out the details with you."
...
export function buildSystemPrompt(config: MotelConfig, now: Date = new Date()): string {
  return `## CURRENT DATE & TIME (server-injected, ${config.identity.timezoneLabel})
Right now it is: ${formatCurrentDate(config, now)}.
